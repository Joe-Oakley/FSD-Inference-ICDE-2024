...
import boto3
from io import BytesIO, StringIO # For loading files from S3
import botocore
from botocore.exceptions import ClientError
# import math
# import ast # Provides handy way of converting "list-like" strings to lists
# import sys
# import time
import timeit # For timing metrics
# import zlib
import csv
...
############### CONSTANTS ###############
SPARSE_DNN_WORKER = 'arn:aws:lambda:eu-west-2:570608630185:function:sparse-dnn-s3-batch-worke-SparseDNNS3BatchWorkerFu-86Qc3lK3dLfi'
...
#COMPRESSION_LEVEL = 5 # Blosc Zstd
    ...
    for batchStart in range(0,numDataToUse,batchSize):
        ...
        for layer in range(numLayers):
            ...
            if layer > 0:
                ...
                # Print size of Z just after deletion of Z[layer - 1]
                # print("Size of W just after deletion: " , sys.getsizeof(W))
                
            #### END VARIABLE REMOVAL ####
        ...
        # print("H[numLayers] shape: ", str(H[numLayers].shape))
        print("H shape: ", str(H.shape))
        
        # my_max_rows = np.ravel(H[numLayers].argmax(axis=0).flatten())           # Dense 1D array. Entries are the ROW NUMBERS of with the highest value for each col)
        ...
        # max_vals_coo = H[numLayers].max(axis=0)                                     # COO matrix (1,20)
...
def output_metrics(s3, metrics_loc, invoc_params, model_params):
    ...
    # s3.put_object(Body=csvio.getvalue(), ContentType='application/vnd.ms-excel', Bucket=bucket, Key=filename)
...
def lambda_handler(event):
    ...
    ######################### READING FROM S3 #########################
    read_weights_start_time = timeit.default_timer()
    readDNNP(weights_loc, model_params)
    read_weights_end_time = timeit.default_timer()
    print("readDNNP done, duration: ", str(read_weights_end_time - read_weights_start_time))
    
    #readConnectivity(connectivity_loc, model_params, invoc_params)
    #print("readConnectivity done")
    
    read_infdata_start_time = timeit.default_timer()
    infDataCSR = readInferenceData(inf_data_loc, model_params)
    read_infdata_end_time = timeit.default_timer()
    print("readInferenceData done, duration: ", str(read_infdata_end_time - read_infdata_start_time))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--jsonloc', type=str, required=True)
